import type { UIActions, AddressActions, TimeActions, Maxtime } from './types'
...
export const createUIActions = (store: Writable<CommuteStore>): UIActions => {
  ...
  return {
    setOpen: (isOpen: boolean) => update(state => ({ ...state, isOpen })),

    toggleOpen: () => update(state => ({ ...state, isOpen: !state.isOpen })),
...
export const createTimeActions = (
  ...
  return {
    setMaxtime: (maxtime: Maxtime) => update(state => ({ ...state, maxtime })),
  }
}
